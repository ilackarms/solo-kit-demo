...
// Deprecated: use PresentationsSyncDeciderWithContext
type PresentationsSyncDecider interface {
	PresentationsSyncer
	ShouldSync(old, new *PresentationsSnapshot) bool
}

type PresentationsSyncDeciderWithContext interface {
	PresentationsSyncer
	ShouldSync(ctx context.Context, old, new *PresentationsSnapshot) bool
}

type presentationsSimpleEventLoop struct {
	emitter PresentationsSimpleEmitter
	syncers []PresentationsSyncer
}

func NewPresentationsSimpleEventLoop(emitter PresentationsSimpleEmitter, syncers ...PresentationsSyncer) eventloop.SimpleEventLoop {
	return &presentationsSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *presentationsSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[PresentationsSyncer]context.CancelFunc)
		...
		var previousSnapshot *PresentationsSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(PresentationsSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(PresentationsSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("presentations.demos.solo.io.SimpleEventLoopSync-%T", syncer))
